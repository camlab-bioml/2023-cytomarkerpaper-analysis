---
title: "Full single-cell analysis NYGC"
format: html
params:
  cell_type: B
---
...
```{r}
hist(df_ct$mean_corrected_sd, breaks=100)
```

```{r}
print(sd(df_ct$mean_corrected_sd))
```
...
Let's read in the NYGC scRNA data:

```{r}
scescrna <- readRDS(here("results/nygc_pbmc_subsampled.rds"))
```


```{r}
# Pull out cell types we have matching equivalent populations in the screen for
sce_ct <- scescrna[, grepl(paste0(screen_scrna_ct_map$str_grep, collapse = "|"), scescrna$cell_type)]
```
...
plotExpression(sce_ct, x = "major_cell_type", features = c("CD3D", "CD4", "CD8A", "MS4A1", "PTPRC", "CD68"))
```
...
sces <- lapply(screen_scrna_ct_map$cell_type, function(ct) {
  ...
  assay(sce_expr, 'seuratNormData') <- assay(sce_expr, 'logcounts')
  sce_expr <- singleCellTK::runSeuratScaleData(sce_expr, useAssay = "logcounts")
  # sce_expr <- singleCellTK::runSeuratFindHVG(sce_expr, useAssay = "logcounts")
  # sce_expr <- singleCellTK::setTopHVG(sce_expr, method = "vst", featureSubsetName = "hvf")
  sce_expr <- singleCellTK::runSeuratPCA(sce_expr, useFeatureSubset=NULL)
  # sce_expr <- singleCellTK::runMNNCorrect(sce_expr, batch = "donor_id")
  sce_expr <- singleCellTK::runSeuratFindClusters(sce_expr, resolution=0.5)
  sce_expr
})
...
fms <- lapply(sces, function(sce) { 
  sce <- sce[rowMeans(logcounts(sce)) > 0.01, ]
  findMarkers(sce, sce$Seurat_louvain_Resolution0.5)
})
```
...
mr_random_screen <- lapply(screen_scrna_ct_map$cell_type, function(ct) {
  fm <- fms[[ct]]
  random_screen_genes <- unique(df_ct$gene_symbol)
...
ggplot(dfj, aes(x = mean_expr, y = mean_corrected_sd, color = log2(best_rank))) +

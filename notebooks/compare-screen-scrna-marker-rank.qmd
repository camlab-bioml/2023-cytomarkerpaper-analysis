...
`dfsum` is the relevant dataframe for cytof
...
```{r}
genes_screen <- unique(dfsum_mapped$gene_symbol)
...
```{r}
dfj <- inner_join(mrkr_rank, dfsum_mapped)
dfj
```
```{r}
...
ggplot(dfj, aes(x = expr_mean, y = expr_sd, color = best_rank > 500)) +
...
```{r}
ggplot(dfj, aes(x = best_rank, y = expr_sd)) +
  geom_point() +
...
mutate(dfj, is_good = best_rank < 500) |> 
  ggplot(aes(x = is_good, y = expr_sd)) +
  geom_boxplot() +
  facet_wrap(~ cell_type, scales = "free") +
  geom_smooth()# +
  # scale_x_log10()
